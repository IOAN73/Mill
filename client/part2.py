...
def handle_color_selection_click(mouse_position, black_player_button, white_player_button):
    global player_color
...
def main_game_loop(game_mode):
    ...
    # Добавляем блок для обработки выбора цвета
    select_color_buttons = draw_select_color_screen()
    pygame.display.flip()
    ...
    while selected_color is None:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                mouse_position = pygame.mouse.get_pos()
                handle_color_selection_click(mouse_position, *select_color_buttons)
                pygame.display.flip()

    # Очищаем экран выбора цвета
    screen.fill(WHITE)
    pygame.display.flip()
...
def main(color):
    ...
    if game_mode:
        ...
        while True:
            color, position = client.get_piece_info()
